...
def get_pets_string(owner_instance):
    title = f"{owner_instance.name.first} {owner_instance.name.last}'s pets are: "
    pets = []
    for pet in owner_instance.pets:
        pets.append(f"{pet.name.first} {pet.name.last}")
    return title + ", ".join(pets)
